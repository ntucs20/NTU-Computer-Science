...
void TestValueOne()
{
	...
	//an example for thread init is shown
	...
	//1b. fork all 4 joinable threads
	//an example for thread fork is shown
	//Inc1->Fork(Inc_v1, 0, 1);
	Inc1->Fork(Inc_v1, 1, 0);
	Inc2->Fork(Inc_v1, 2, 0);
	Dec1->Fork(Dec_v1, 3, 0);
	Dec2->Fork(Dec_v1, 4, 1);

	//1c. join all the threads (currentThread here refers to main thread)
	//an example is shown where main thread wait for thread Inc1 to finish running
...
void TestValueMinusOne()
{
	...
	Inc1->Fork(Inc_v2, 1, 0);
	Inc2->Fork(Inc_v2, 2, 0);
	Dec1->Fork(Dec_v2, 3, 0);
	Dec2->Fork(Dec_v2, 4, 1);
...
void TestConsistency()
{
	...
	//1b. fork all 4 joinable threads
	//value should be consistent regardless of reder of thread fork
	//Inc1->Fork(Inc_Consistent, 0, 1);
	Inc1->Fork(Inc_Consistent, 1, 1);
	Inc2->Fork(Inc_Consistent, 2, 1);
	Dec1->Fork(Dec_Consistent, 3, 1);
	Dec2->Fork(Dec_Consistent, 4, 1);
