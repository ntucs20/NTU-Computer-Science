...
Thread::Finish ()
{
	...
	//remove pending timer interrupt from pending list. this is the interrrupt based on fixed time quantum
	interrupt->RemoveFromPendingQueue();

	//reschedule a new timer interrupt so that it is triggered after a fixed time quantum from current time
	interrupt->Schedule(timer->getHandler(), (_int) timer, timer->TimeOfNextInterrupt(), TimerInt); 
	
	//debug msg to denote resetting of time interrrupt	
